...
import com.gracefinance.gracefinanceapp.repository.DepenseSqlHelper;
...
import com.gracefinance.gracefinanceapp.repository.rowmapper.DepenseRowMapper;
...
/**
 * Spring Data R2DBC custom repository implementation for the Depense entity.
 */
@SuppressWarnings("unused")
class DepenseRepositoryInternalImpl extends SimpleR2dbcRepository<Depense, Long> implements DepenseRepositoryInternal {
    ...
    private final DepenseRowMapper depenseMapper;
    private final ColumnConverter columnConverter;

    private static final Table entityTable = Table.aliased("depense", EntityManager.ENTITY_ALIAS);
    ...
    public DepenseRepositoryInternalImpl(
        R2dbcEntityTemplate template,
        EntityManager entityManager,
        EntiteFinanciereRowMapper entitefinanciereMapper,
        CaisseRowMapper caisseMapper,
        CategorieRowMapper categorieMapper,
        DepenseRowMapper depenseMapper,
        R2dbcEntityOperations entityOperations,
        R2dbcConverter converter,
        ColumnConverter columnConverter
    ) {
        super(
            new MappingRelationalEntityInformation(converter.getMappingContext().getRequiredPersistentEntity(Depense.class)),
            entityOperations,
            converter
        );
    ...
    @Override
    public Flux<Depense> findAllBy(Pageable pageable) {
        return createQuery(pageable, null).all();
    }

    RowsFetchSpec<Depense> createQuery(Pageable pageable, Condition whereClause) {
        List<Expression> columns = DepenseSqlHelper.getColumns(entityTable, EntityManager.ENTITY_ALIAS);
        ...
        String select = entityManager.createSelect(selectFrom, Depense.class, pageable, whereClause);
        return db.sql(select).map(this::process);
    }

    @Override
    public Flux<Depense> findAll() {
        return findAllBy(null);
    }

    @Override
    public Mono<Depense> findById(Long id) {
        Comparison whereClause = Conditions.isEqual(entityTable.column("id"), Conditions.just(id.toString()));
        return createQuery(null, whereClause).one();
    }

    private Depense process(Row row, RowMetadata metadata) {
        Depense entity = depenseMapper.apply(row, "e");
    ...
    @Override
    public <S extends Depense> Mono<S> save(S entity) {
        return super.save(entity);
    }

    @Override
    public Flux<Depense> findByCriteria(DepenseCriteria depenseCriteria, Pageable page) {
        return createQuery(page, buildConditions(depenseCriteria)).all();
    }
    ...
    private Condition buildConditions(DepenseCriteria criteria) {
        ...
        if (criteria != null) {
            ...
            if (criteria.getDateDepense() != null) {
                builder.buildFilterConditionForField(criteria.getDateDepense(), entityTable.column("date_depense"));
            }
