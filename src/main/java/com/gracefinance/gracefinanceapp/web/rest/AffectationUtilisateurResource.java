...
/**
 * REST controller for managing {@link com.gracefinance.gracefinanceapp.domain.security.AffectationUtilisateur}.
 */
@RestController
...
public class AffectationUtilisateurResource {
     ...
     */
    @PostMapping("")
    public Mono<ResponseEntity<AffectationUtilisateurDTO>> createAffectationUtilisateur(
        ...
        return affectationUtilisateurService
            .save(affectationUtilisateurDTO)
            .map(result -> {
                try {
                    return ResponseEntity.created(new URI("/api/affectation-utilisateurs/" + result.getId()))
     ...
     */
    @PutMapping("/{id}")
    public Mono<ResponseEntity<AffectationUtilisateurDTO>> updateAffectationUtilisateur(
        ...
        return affectationUtilisateurRepository
            ...
            .flatMap(exists -> {
                ...
                return affectationUtilisateurService
                    .update(affectationUtilisateurDTO)
    ...
    public Mono<ResponseEntity<AffectationUtilisateurDTO>> partialUpdateAffectationUtilisateur(
        @PathVariable(value = "id", required = false) final Long id,
        ...
        return affectationUtilisateurRepository
            ...
            .flatMap(exists -> {
                ...
                Mono<AffectationUtilisateurDTO> result = affectationUtilisateurService.partialUpdate(affectationUtilisateurDTO);
    ...
    /**
     * {@code GET  /affectation-utilisateurs} : get all the affectationUtilisateurs.
     *
     * @param pageable the pagination information.
     * @param request a {@link ServerHttpRequest} request.
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of affectationUtilisateurs in body.
     */
    @GetMapping(value = "", produces = MediaType.APPLICATION_JSON_VALUE)
    public Mono<ResponseEntity<List<AffectationUtilisateurDTO>>> getAllAffectationUtilisateurs(
        AffectationUtilisateurCriteria criteria,
        ...
        return affectationUtilisateurService
            ...
            .zipWith(affectationUtilisateurService.findByCriteria(criteria, pageable).collectList())
     ...
     */
    @GetMapping("/count")
    public Mono<ResponseEntity<Long>> countAffectationUtilisateurs(AffectationUtilisateurCriteria criteria) {
        LOG.debug("REST request to count AffectationUtilisateurs by criteria: {}", criteria);
        return affectationUtilisateurService.countByCriteria(criteria).map(count -> ResponseEntity.status(HttpStatus.OK).body(count));
    }
     ...
     */
    @GetMapping("/{id}")
    public Mono<ResponseEntity<AffectationUtilisateurDTO>> getAffectationUtilisateur(@PathVariable("id") Long id) {
        LOG.debug("REST request to get AffectationUtilisateur : {}", id);
        Mono<AffectationUtilisateurDTO> affectationUtilisateurDTO = affectationUtilisateurService.findOne(id);
     ...
     */
    @DeleteMapping("/{id}")
    public Mono<ResponseEntity<Void>> deleteAffectationUtilisateur(@PathVariable("id") Long id) {
