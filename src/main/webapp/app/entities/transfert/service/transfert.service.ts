...
type RestOf<T extends ITransfert | NewTransfert> = Omit<T, 'dateTransfert' | 'dateValidation'> & {
...
export class TransfertService {
  ...
  protected convertDateFromClient<T extends ITransfert | NewTransfert | PartialUpdateTransfert>(transfert: T): RestOf<T> {
    return {
      ...
      dateValidation: transfert.dateValidation?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restTransfert: RestTransfert): ITransfert {
    return {
      ...
      dateValidation: restTransfert.dateValidation ? dayjs(restTransfert.dateValidation) : undefined,
    };
  }
